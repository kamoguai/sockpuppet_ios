...
extension HttpProxyProtocol: URLSessionDataDelegate {
    ...
    {
        print("=enter URLSessionDataDelegate URLSessionDataTask completionHandler=")
        client?.urlProtocol(self, didReceive: response, cacheStoragePolicy: .notAllowed)
        completionHandler(.allow)
    }
    
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        print("=enter URLSessionDataDelegate URLSessionDataTask=")
...
extension HttpProxyProtocol: URLSessionTaskDelegate {
    func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse, newRequest request: URLRequest, completionHandler: @escaping (URLRequest?) -> Void) {
        print("=enter URLSessionTaskDelegate willPerformHTTPRedirection=")
        if let request = request as? NSMutableURLRequest {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        print("=enter URLSessionTaskDelegate URLSessionTask=")
    ...
    func urlSession(_ session: URLSession, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        print("== print httpproxyprotocol session auth challenge ==")
        if enableProxy {
